...
export const MOTOR_PRICES = {
  ...
  hub: { original: 185, marked: Math.round(185 * 1.25) },
  sunSensor: { original: 150, marked: Math.round(150 * 1.25) },
  charger: { original: 50, marked: Math.round(50 * 1.25) },
};
...
export const VALANCE_OPTIONS = [
  { id: 'standard', label: 'Standard Roll', pricePerInch: 0, desc: 'Simple & clean — fabric rolls from the back' },
  { id: 'reverse', label: 'Reverse Roll', pricePerInch: 0, desc: 'Fabric rolls from the front for a flush look' },
  { id: 'cassette', label: 'Cassette', pricePerInch: 4.00, desc: 'Clean, enclosed look — hides the roller' },
...
export const SIDE_CHANNEL_OPTIONS = [
  ...
  { id: 'standard', label: 'Light Blocking Side Channels', pricePerFoot: 15 }
];
...
// ─── STARTING PRICE HELPERS ───────────────────────
// Min price group per category (derived from FABRIC_PRICE_GROUPS)
// LF cheapest: B (SheerGuard, PrivacyLite, RenewWeave, PureShade, Tranquil Haven, Coastal Luxe LF)
// BO cheapest: B (BreezeGuard, DayLite Max, ArmorWeave)
const MIN_GROUP: Record<string, string> = { 'Light Filtering': 'B', 'Blackout': 'B' };

export const getStartingPrice = (
  shape: ShapeType,
  category?: 'Light Filtering' | 'Blackout',
  w?: number,
  h?: number
) => {
  const isSpecialty = shape !== 'Standard';
  const width = w && w > 0 ? w : 36;
  const height = h && h > 0 ? h : 36;
  const group = category ? (MIN_GROUP[category] || 'B') : 'B';
  const marked = isSpecialty
    ? getPriceFromTable('', width, height, true)
    : getPriceFromTable(group, width, height, false);
  const sale = isSaleActive() ? getSalePrice(marked) : marked;
  return { marked, sale, saleActive: isSaleActive() };
};
