...
interface VisualizerProps {
  ...
  shape?: ShapeType; 
  isCollapsed?: boolean;
  onSnapshotReady?: (dataUrl: string) => void; // called after shade overlay renders
}
...
const Visualizer: React.FC<VisualizerProps> = ({
  ...
  useEffect(() => {
    ...
    // Snapshot: composite room image + shade overlay for cart preview
    if (onSnapshotReadyRef.current && selection && imgRef.current && !imageError) {
      try {
        const snapCanvas = document.createElement('canvas');
        snapCanvas.width = metrics.cW;
        snapCanvas.height = metrics.cH;
        const snapCtx = snapCanvas.getContext('2d');
        if (snapCtx) {
          snapCtx.drawImage(imgRef.current, metrics.offsetX, metrics.offsetY, metrics.renderW, metrics.renderH);
          snapCtx.drawImage(canvas, 0, 0);
          onSnapshotReadyRef.current(snapCanvas.toDataURL('image/jpeg', 0.75));
        }
      } catch (e) {
        // CORS/taint issue — skip snapshot silently
      }
    }
  ...
  return (
    <div className="flex flex-col gap-4 flex-1 min-h-0">
      ...
      <div className={`${isCollapsed ? 'hidden md:flex' : 'flex'} flex-wrap gap-3 justify-between items-center pb-3 relative shrink-0 transition-all duration-300`}>
        ...
        <p className="text-[11px] text-[#999] w-full -mt-2">Your shade updates as you customize</p>
          ...
          <>
             {imageError ? (
               ...
               <img 
                 ...
                 className="absolute inset-0 w-full h-full object-contain select-none bg-transparent pointer-events-none z-0 visualizer-room-img"
                 style={{ 
