...
import { getFabricUrl, SHAPE_CONFIGS, VALANCE_OPTIONS, SIDE_CHANNEL_OPTIONS, isSaleActive, SALE_CONFIG } from '../constants';
...
const CartItemCard: React.FC<{
  ...
  if (item.config.shape === 'Standard') {
    ...
    dimDisplay = SHAPE_CONFIGS[item.config.shape as ShapeType].inputs.map((input: any) => {
      let val = 0;
      let frac = '0';
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 animate-in slide-in-from-right-4 duration-300">
      {/* Room preview image — full width if available, else fabric swatch */}
      {item.visualizerImage ? (
        <div className="relative w-full" style={{ paddingBottom: '52%' }}>
          <img
            src={item.visualizerImage}
            alt="Room preview"
          ...
          <button
            onClick={onRemove}
            className="absolute top-2 left-2 bg-black/40 hover:bg-red-500 text-white rounded-full p-1 transition-colors"
          >
            <X size={12} />
          </button>
        </div>
      ) : (
        /* Fallback: fabric swatch strip */
        <div className="relative flex items-center gap-3 p-3 bg-gray-50 border-b border-gray-100">
          <div className="w-14 h-14 rounded-lg bg-gray-100 shrink-0 border border-gray-200 overflow-hidden">
            {item.config.material
              ? <img src={getFabricUrl(item.config.material.cloudinaryId, 'thumb')} alt="Fabric" className="w-full h-full object-cover" />
              : <div className="w-full h-full flex items-center justify-center bg-indigo-50 text-indigo-200"><Ruler size={20} /></div>
            }
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-bold text-slate-800 truncate">
              {item.config.material?.name || 'Custom Shade'}
            </div>
            <div className="text-[11px] text-slate-400 mt-0.5">{dimDisplay}</div>
          </div>
          <div className="flex flex-col items-end gap-1 shrink-0">
            <div className="text-[10px] bg-slate-100 text-slate-600 font-bold px-2 py-0.5 rounded-full">Qty {item.config.quantity}</div>
            <button onClick={onRemove} className="text-gray-300 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
          </div>
        </div>
      )}

      {/* Price row + collapsible toggle */}
      <div className="flex items-center justify-between px-3 py-2.5">
        <div className="font-bold text-slate-900 text-[15px]">${item.totalPrice.toFixed(2)}</div>
        <button
          onClick={() => setExpanded(e => !e)}
          className="flex items-center gap-1 text-[11px] font-medium text-slate-400 hover:text-slate-600 transition-colors"
        >
          {expanded ? 'Hide details' : 'View your configuration'}
      ...
      {expanded && (
        <div className="px-3 pb-3 border-t border-gray-50" style={{ animation: 'slideDown 0.2s ease forwards' }}>
          <table className="w-full text-[11px] mt-2">
            <tbody>
              {specs.map(({ label, value }) => (
                <tr key={label} className="border-b border-gray-50 last:border-0">
                  <td className="py-1 pr-3 font-medium text-slate-400 uppercase tracking-wider text-[9px] whitespace-nowrap">{label}</td>
                  <td className="py-1 text-slate-700 font-medium">{value}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {step === 'cart' && (
            <>
                ...
                {cart.length > 0 ? (
                    ...
                    {cart.map((item) => item.config.isMeasurementOnly ? (
                        // Measurement-only items keep simple layout
                        <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex gap-3">
                          <div className="flex-1">
                            <div className="flex justify-between items-start">
                              <h4 className="font-bold text-slate-800 text-sm">{t('pro.measureOnly')}</h4>
                              <button onClick={() => onRemoveItem(item.id)} className="text-gray-400 hover:text-red-500"><Trash2 size={14} /></button>
                            </div>
                            <div className="text-xs text-slate-500 mt-1">{t('pro.location')}: {item.config.installer?.location}</div>
                            <div className="text-xs text-green-600 font-medium mt-0.5">Credited back on order</div>
                            <div className="font-bold text-slate-900 text-sm mt-2">${item.totalPrice.toFixed(2)}</div>
                          </div>
                        </div>
                    ) : (
                        <CartItemCard
                ...
                <div className="p-5 bg-white border-t border-gray-100 shadow-xl z-10 shrink-0 space-y-3">
                    <div className="flex justify-between items-end mb-1">
                        <div>
                          <span className="text-sm font-medium text-slate-500 block">Total <span className="text-green-600 font-semibold">(Free Shipping Included)</span></span>
                    ...
                    <div className="flex items-center gap-1.5 text-[11px] text-green-700 font-medium bg-green-50 rounded-lg px-3 py-2">
                        ...
                        Free shipping • Estimated delivery in 7–10 business days • Made in the USA
                    </div>
                    {/* Urgency */}
                    {isSaleActive() && (
                      <div className="text-center">
                        <p className="text-[11px] font-semibold text-[#c0593a]">
                          ⏳ Sale ends soon — don't miss your discount
                        </p>
                      </div>
                    )}
                    <button onClick={handleProceedToCheckout} disabled={isProcessing || (cart.length === 0 && swatches.length === 0)} className="w-full text-white py-4 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed" style={{ background: 'linear-gradient(90deg, #C8A165 0%, #E7D8B8 55%, #C8A165 100%)', boxShadow: '0 10px 25px rgba(212, 175, 55, 0.35)', color: '#1a1a1a' }}>
                        {isProcessing ? (
                    ...
                    {/* Reassurance */}
                    <p className="text-center text-[10px] text-[#bbb]">You can review and edit before checkout</p>
                    ...
                    <div className="flex items-center justify-center gap-4 flex-wrap pt-1">
                        ...
                        <span className="text-[10px] text-[#666] flex items-center gap-1">
                          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg>
