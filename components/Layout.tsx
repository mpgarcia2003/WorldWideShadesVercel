    ...
    <div className="h-screen w-screen flex flex-col font-sans bg-white text-slate-900 overflow-hidden">
      ...
      <header className="h-16 bg-white border-b border-gray-200 text-slate-900 shrink-0 z-[60] relative">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-full flex items-center justify-between">
          ...
          {/* Sale Banner — inline nav pill */}
          {isSaleActive() && (
            <div className="hidden md:flex items-center gap-3 px-4 py-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, #1a1a1a 0%, #2d2418 50%, #1a1a1a 100%)', border: '1px solid rgba(200,161,101,0.3)' }}>
              <div>
                <div className="text-[12px] font-medium text-white tracking-wide" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
                  {t('sale.banner')}
                </div>
                <div className="text-[10px] text-[#c8a165] font-medium">
                  {t('sale.ends')}
                </div>
              </div>
              <div className="px-2.5 py-1 rounded-lg text-[11px] font-black tracking-wider" style={{ background: 'linear-gradient(90deg, #C8A165, #E7D8B8, #C8A165)', color: '#1a1a1a' }}>
                {t('sale.badge')}
              </div>
            </div>
          )}
