 ...
 *   4. Total + CTA — sticky at bottom, mirrors StickyBottomBar on mobile.
 *
...
const BuilderRightRail: React.FC<BuilderRightRailProps> = ({
  ...
  return (
    <div className="hidden lg:flex w-[26%] flex-col h-full bg-[#FDFBF7] border-l border-gray-100 shrink-0 overflow-hidden">
      <div className="flex-1 overflow-y-auto p-4 pb-2">
        {/* ─── 1. STEP GUIDE ─────────────────────────── */}
        <div className="bg-white rounded-2xl p-4 mb-3" style={{ border: '1px solid rgba(20,20,20,0.06)', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: '#f5f0e6' }}>
              <Icon size={16} className="text-[#8b6d3f]" />
            </div>
          ...
          <h3 className="text-[15px] font-medium text-[#1a1a1a] leading-tight mb-2" style={{ fontFamily: "'Playfair Display', Georgia, serif", letterSpacing: '-0.01em' }}>
            {guide.headline}
          </h3>
          ...
          {guide.tips.length > 0 && (
            <div className="space-y-1.5 pt-2 border-t border-gray-100">
              {guide.tips.map((tip, i) => (
                <div key={i} className="flex items-start gap-2 text-[11px] text-[#555] leading-relaxed">
          ...
          {guide.footer && (
            <div className="mt-3 pt-3 border-t border-gray-100 text-[11px] font-medium text-[#8b6d3f] italic leading-relaxed">
              {guide.footer}
            </div>
          )}
        </div>
        ...
        {/* ─── 2. NEED HELP CARD ──────────────────────── */}
        <div className="rounded-2xl p-4 mb-3" style={{ backgroundColor: '#fdfaf3', border: '1px solid #e8dcc8' }}>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: '#c8a165' }}>
          ...
          <a href="tel:+18446742716" className="block text-[15px] font-semibold text-[#8b6d3f] hover:text-[#a8844d] transition-colors mb-1">
            (844) 674-2716
          </a>
        ...
        <div className="bg-white rounded-2xl p-4 mb-3" style={{ border: '1px solid rgba(20,20,20,0.06)' }}>
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#aaa] mb-3">
            Why World Wide Shades
          </div>
          <div className="space-y-2.5">
            <div className="flex items-center gap-2.5">
              <Truck size={14} className="text-[#c8a165] shrink-0" strokeWidth={1.5} />
              <span className="text-[11px] text-[#555] font-medium">Free shipping on every order</span>
            </div>
            <div className="flex items-center gap-2.5">
              <ShieldCheck size={14} className="text-[#c8a165] shrink-0" strokeWidth={1.5} />
              <span className="text-[11px] text-[#555] font-medium">100% perfect fit guarantee</span>
            </div>
            <div className="flex items-center gap-2.5">
              <Calendar size={14} className="text-[#c8a165] shrink-0" strokeWidth={1.5} />
              <span className="text-[11px] text-[#555] font-medium">Ships in 5–7 business days</span>
            </div>
            <div className="flex items-center gap-2.5">
              <Award size={14} className="text-[#c8a165] shrink-0" strokeWidth={1.5} />
      ...
      {/* ─── 4. STICKY TOTAL + CTA ───────────────────── */}
      <div className="bg-white border-t border-gray-100 p-4 shrink-0" style={{ boxShadow: '0 -2px 8px rgba(0,0,0,0.03)' }}>
        {totalPrice > 0 ? (
          <>
            {saleActive && savings > 0 && (
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-[12px] text-[#bbb] line-through">${originalPrice.toFixed(0)}</span>
            ...
            <div className="flex items-baseline gap-1 mb-1">
              <span className="text-[24px] font-medium text-[#1a1a1a]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
                ${totalPrice.toFixed(2)}
              </span>
            </div>
            <div className="flex items-center gap-1 text-[10px] text-[#888] mb-3">
          ...
          <div className="mb-3">
            <div className="text-[12px] font-medium text-[#999]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
              Your total
            </div>
        ...
        <button
          ...
          className="w-full py-3 rounded-xl font-medium text-[13px] tracking-wide transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          style={isStepValid ? {
            background: 'linear-gradient(90deg, #C8A165 0%, #E7D8B8 55%, #C8A165 100%)',
            boxShadow: '0 4px 16px rgba(200,161,101,0.2)',
            color: '#1a1a1a',
            fontFamily: "'Playfair Display', Georgia, serif",
          } : { backgroundColor: '#ede8df', color: '#aaa' }}
        >
          {isLastStep ? 'Add to Cart' : 'Continue'}
          {isStepValid && <ChevronRight size={15} />}
        </button>
        <p className="text-center text-[9px] text-[#bbb] mt-2 font-medium">Secure checkout · Easy returns</p>
      </div>
    </div>
  );
};
